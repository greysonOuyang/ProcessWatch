...
final class AnomalyDetector {
    ...
    func evaluate(
        ...
        evaluateGroups(ProcessGroupSnapshot.grouped(processes), settings: settings, now: now, generated: &generated)

        let activeProcesses = processTrackers.reduce(into: [Int32: Set<AnomalyKind>]()) { result, entry in
    ...
    private func evaluateProcesses(
        ...
        for process in processes {
            if settings.isIgnored(process.name) {
            ...
            evaluateMemory(process, tracker: &tracker, settings: settings, now: now, generated: &generated)
    ...
    private func evaluateGroups(
        ...
        for group in groups {
            ...
            if isStorm {
                ...
                if let since = tracker.stormSince,
                   ...
                   shouldAlert(.processStorm, tracker: tracker, now: now) {
                    generated.append(makeGroupEvent(
                        group: group,
                        kind: .processStorm,
                        title: "检测到进程风暴",
                        detail: "同一可执行文件存在 \(group.instanceCount) 个实例，CPU 合计 \(formatPercent(group.cpuPercent))，内存合计 \(formatBytes(group.memoryBytes))，其中孤儿进程 \(group.orphanCount) 个，已持续约 \(formatDuration(now.timeIntervalSince(since)))。"
                    ))
            ...
            let repoHarnessOrphans = group.processes.filter {
                $0.isOrphan &&
                    $0.affiliation.belongsToRepoHarness &&
            ...
            if isRepoHarnessLeak {
                if shouldAlert(.repoHarnessLeak, tracker: tracker, now: now) {
                    generated.append(makeGroupEvent(
                        ...
                        detail: "检测到 \(repoHarnessOrphans.count) 个运行超过 \(formatDuration(settings.repoHarnessOrphanDuration)) 的孤儿 \(group.name) 进程；同一可执行文件当前共有 \(group.instanceCount) 个实例。"
                    ))
        ...
        if process.cpuPercent >= settings.cpuThreshold {
            ...
            if let since = tracker.cpuHighSince,
               ...
               shouldAlert(.cpu, tracker: tracker, now: now) {
                generated.append(makeEvent(
                    process: process,
                    kind: .cpu,
                    title: "发现持续高 CPU 进程",
                    detail: "CPU \(formatPercent(process.cpuPercent))，已持续约 \(formatDuration(now.timeIntervalSince(since)))。"
                ))
            ...
            if let since = tracker.diskHighSince,
               ...
               shouldAlert(.diskWrite, tracker: tracker, now: now) {
                generated.append(makeEvent(
                    ...
                    kind: .diskWrite,
                    title: "发现持续写盘进程",
                    detail: "当前写入 \(formatRate(process.diskWriteBytesPerSecond))，已持续约 \(formatDuration(now.timeIntervalSince(since)))。"
                ))
    ...
    private func evaluateMemory(
        ...
        let retentionStart = now.addingTimeInterval(-settings.memoryWindow - max(30, settings.samplingInterval * 3))
        tracker.memoryPoints.removeAll { $0.date < retentionStart }

        guard let baseline = tracker.memoryPoints.first,
              now.timeIntervalSince(baseline.date) >= settings.memoryWindow * 0.9 else {
            return
        }
        ...
        if growth >= threshold {
            if shouldAlert(.memoryGrowth, tracker: tracker, now: now) {
                generated.append(makeEvent(
                    ...
                    kind: .memoryGrowth,
                    title: "发现内存持续增长进程",
                    detail: "约 \(formatDuration(now.timeIntervalSince(baseline.date))) 内增长 \(formatBytes(growth))，当前占用 \(formatBytes(process.memoryBytes))。"
                ))
